...
{ # Hinge findind Algorithm
  ...
  EX_p= D %*% diag(E)
  EX_p=(EX_p[,which(d %*% D >=0)]) %*% matrix(1,nrow=ncol(S_p),ncol=1)
  
  EX_n= D %*% diag(E)
  EX_n=(EX_n[,which(d %*% D <0)]) %*% matrix(1,nrow=ncol(S_n),ncol=1)
  
  B=solve(L_p) %*% L_10+solve(L_n) %*% L_01
  d=d_f-B %*% d_f+solve(L_p) %*% EX_p-solve(L_n) %*% EX_n
